...
describe("IMAGES: test user image upload", () => {
    it("IMAGES: test normal functionality", () => {
        const resp1 = request(app).post('/users/createUser')
        const uid = resp1.body.uid;
        const resp2 = request(app).post('/images/userProfilePic/upload')
            .field('uid', uid)
            .attach('img', 'test/testImages/test1.jpg')
        assert.strictEqual(resp2.status, 200)
        const resp3 = request(app).get('/users/selectUser')
            .send('uid', uid)
        assert.strictEqual(resp3.body.pfpLink, resp2.body.pfpLink)
    });
})
